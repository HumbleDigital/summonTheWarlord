...
async function verifySwap(raptor, txid, debugLog) {
  const check = async () => {
    try {
      const details = await raptor.getTransactionStatus(txid);
      await debugLog?.write("raptor.transaction_status", { response: details });
      ...
      if (status === "failed" || status === "failure" || status === "error") {
        throw new SwapError(
          `Raptor reported transaction failure: ${formatRaptorResponseError(details, details?.status || "failed")}`,
          { details: { raptor: details } }
        );
      }
      if (status === "expired") {
        throw new SwapError(
          `Raptor reported transaction expired: ${formatRaptorResponseError(details, "expired")}`,
          { details: { raptor: details } }
        );
      }
      return "pending";
...
async function executeSwap({
  ...
  try {
    ...
    await debugLog?.write("raptor.transaction_accepted", { txid, signatureMatch });
    if (!signatureMatch) {
      throw new SwapError("Raptor accepted a transaction with a different signature than the signed payload.");
    }
    await debugLog?.write("raptor.transaction_sent", { txid });
  } catch (err) {
  ...
  try {
    const verified = await verifySwap(raptor, txid, debugLog);
    if (!verified) {
      await debugLog?.write("raptor.transaction_unconfirmed", { txid });
      throw new SwapError(`Transaction ${txid} was submitted but not confirmed by Raptor.`);
    }
