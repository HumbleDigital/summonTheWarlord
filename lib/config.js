...
/** Raptor priorityFee level names (docs: Min/Low/Auto/Medium/High/VeryHigh/Turbo/UnsafeMax). */
...
export const DEFAULT_CONFIG = {
  ...
  // Fixed percent, or "dynamic" (Raptor keyword; "auto" accepted and normalized to "dynamic")
  slippage: 10,
  // "auto" defers to priorityFeeLevel; level names and microlamports also valid
  priorityFee: "auto",
  // Raptor recommended default tier is Auto (with Medium as peer)
  priorityFeeLevel: "auto",
  maxPriorityFee: null,
...
function evaluateConfigValue(key, value) {
  if (key === "slippage") {
    if (typeof value === "string") {
      const lowered = value.trim().toLowerCase();
      // Raptor API uses "dynamic"; accept CLI alias "auto" and normalize to "dynamic".
      if (lowered === "auto" || lowered === "dynamic") {
        return { normalized: "dynamic" };
      }
    }
    const num = coerceNumber(value);
    if (num === null || num < 0) {
      return { error: "Invalid slippage. Use a non-negative percent or \"dynamic\" (alias: \"auto\")." };
    }
  ...
  if (key === "priorityFee") {
    if (typeof value === "string") {
      ...
      // "auto" means: use priorityFeeLevel (which may itself be Raptor's "auto" tier).
      if (lowered === "auto") return { normalized: "auto" };
      const level = PRIORITY_FEE_LEVELS.find((l) => l.toLowerCase().replace(/_/g, "") === lowered);
      if (level) return { normalized: level };
    }
    const num = coerceNumber(value);
    if (num === null || num < 0) {
      return {
        error: `Invalid priorityFee. Use "auto", a level (${PRIORITY_FEE_LEVELS.join(", ")}), or microlamports.`,
      };
    }
    return { normalized: num };
  }
  if (key === "priorityFeeLevel") {
    ...
    const match = PRIORITY_FEE_LEVELS.find(
      (level) => level.toLowerCase().replace(/_/g, "") === value.trim().toLowerCase().replace(/_/g, "")
    );
    if (!match) {
      return { error: `Invalid priorityFeeLevel. Use one of ${PRIORITY_FEE_LEVELS.join(", ")}.` };
    }
