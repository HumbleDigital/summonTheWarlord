...
/** Public mainnet RPC used only for sendTransaction when the quote RPC is SolanaTracker. */
export const DEFAULT_SEND_TRANSACTION_ENDPOINT =
  "https://api.mainnet-beta.solana.com";

/**
 * SolanaTracker quote/RPC hosts often cannot send (`no proxy available`).
 * Prefer an explicit sendRpcUrl; otherwise route ST hosts through public mainnet.
 */
export function resolveSendTransactionEndpoint(rpcUrl, sendRpcUrl) {
  if (typeof sendRpcUrl === "string" && sendRpcUrl.trim() !== "") {
    return sendRpcUrl.trim();
  }
  ...
  try {
    ...
    if (hostname === "solanatracker.io" || hostname.endsWith(".solanatracker.io")) {
      return DEFAULT_SEND_TRANSACTION_ENDPOINT;
    }
  } catch {
    return null;
  }
...
async function defaultFactory(options = {}) {
  ...
  const [{ Keypair }, { parsePrivateKeyToSecretKey }] = await Promise.all([
    import("@solana/web3.js"),
    import("./privateKey.js"),
  ]);

  // Do NOT wipe secretBytes after fromSecretKey: web3.js Keypair retains the
  // same Uint8Array reference for secretKey (no copy). Zeroing it would break
  // all signing for the process lifetime.
  const secretBytes = parsePrivateKeyToSecretKey(raw);
  const keypair = Keypair.fromSecretKey(secretBytes);

  const rpcUrl = ensureAdvancedTx(cfg.rpcUrl);
  const apiKey = SWAP_DISCOUNT_CODE;
  // Debug only when explicitly enabled in config — never via NODE_ENV
  const debugEnabled = Boolean(cfg.DEBUG_MODE);
  ...
  try {
    ...
    const sendEndpoint = resolveSendTransactionEndpoint(cfg.rpcUrl, cfg.sendRpcUrl);
    if (
      sendEndpoint
      && typeof tracker.setCustomSendTransactionEndpoint === "function"
    ) {
      await tracker.setCustomSendTransactionEndpoint(sendEndpoint);
    }
    return tracker;
  } catch (err) {
