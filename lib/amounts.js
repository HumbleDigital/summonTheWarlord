...
/**
 * Map CLI/config slippage to Raptor `slippageBps`.
 * Raptor accepts numeric bps or the string `"dynamic"`.
 * Config may store percent numbers, or `"dynamic"` (alias `"auto"` normalized at config load).
 * @param {number|string} slippage
 * @returns {string}
 */
export function toSlippageBps(slippage) {
  if (typeof slippage === "string") {
    const lowered = slippage.trim().toLowerCase();
